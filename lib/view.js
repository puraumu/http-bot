...
function View() {
  ...
  // verbose settings
  this.verboseRequest = false;
  this.verboseWarning = false;
  // statusCode
  this.success = false; // 2xx
  this.redirection = false; // 3xx
  this.clientError = false; // 4xx
  this.serverError = false; // 5xx
};
...
View.prototype.appendRequestLog = function (headers) {
  var dir = this.dir
    , str = ''
...
/**
 * Verbose debugging. Both `ClientRequest` object and `request`'s `request` object
 * should work.
 *
 * @param {ClientRequest}
 */
...
View.prototype.showVerboseRequest = function (req) {
  var stout = '';
  if (objName != undefined) {
    stout += objName + ' ' + util.inspect(req[objName], false, null, true)
  } else {
    if (req._headers != undefined) {
      objName = '_headers';
      return this.showVerboseRequest(req);
    };
    if (req.headers != undefined) {
      objName = 'headers';
      return this.showVerboseRequest(req);
    };
    objName = 'no "headers" object';
    return this.showVerboseRequest(req);
  };
  stout += '\n';
  if (strName != undefined) {
    stout += strName + ' ' + util.inspect(req[strName], false, null, true)
  } else {
    if (req._header != undefined)  {
      strName = '_header';
      return this.showVerboseRequest(req);
    };
...
View.prototype.showVerboseResponse = function (res) {
  var stout = '';
  stout += 'headers' + ' ' + util.inspect(res.headers, false, null, true)
  console.log(stout);
};
...
View.prototype.displayResponse = function(headers, url, duration) {
  ...
  if (399 < headers.statusCode) exist = color('fail', '✖');
  if (1000 < duration) {
    if (60000 < duration) {
      // minutes
...
View.prototype.appendResponseLog = function (headers) {
  var dir = this.dir
    , str = ''
