...
file.checkUrl = function(res) {
  ...
  if (res.body == '') {
    urls[index].title = now()
  } else {
    var title = res.body.match(/<title>(.*)<\/title>/)
    if (title !== null) urls[index].title = title[1]
    else urls[index].title = now()
  };
  index += 1
  // TODO
  if (index == 0) return // no such url in urls
...
file.tryRequest = function() {
  var self = this
    , url = this.urls[this.index].data
...
function createAction(set) {
  action.protoact = checkActionType(set.act);
}
