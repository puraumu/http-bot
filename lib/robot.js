...
exports.display = function() {
  ...
  if (view.request) {
    log.on('req', function(req, url) {
      view.displayRequest(req, url)
    });
  };
  if (view.response) {
    log.on('res', function(res, url, duration) {
      view.displayResponse(res, url, duration)
    });
  };
  if (view.warning) {
    log.on('notfound', function(req, url) {
      view.displayRequest(req, url)
    });
  };
  return this;
...
exports.start = function(name, write) {
  override.call(exports);
  action.trigger(name, write);
  return this;
};
