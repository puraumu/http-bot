...
exports.display = function() {
  ...
  var code;
  if (view.serverError) code = 499;
  if (view.clientError) code = 399;
  if (view.redirection) code = 299;

  if (view.request) {
    log.on('req', function(req) {
      view.displayRequest(req, req.url)
    });
    if (view.verboseRequest) {
      log.on('req', function(req) {
        view.showVerboseRequest(req)
      });
    };
  };
  ...
  if (view.response) {
    ...
    if (view.success) {
      log.on('res', function(res) {
        view.showVerboseResponse(res)
      });
    } else {
      if (view.redirection || view.clientError || view.serverError) {
        log.on('res', function(res) {
